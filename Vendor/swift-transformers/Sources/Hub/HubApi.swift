    ...
    struct HubFileDownloader {
        ...
        func download(progressHandler: @escaping (Double, Double?) -> Void) async throws -> URL {
            ...
            try await withTaskCancellationHandler {
                let sub = await downloader.download(from: source, using: hfToken, expectedSize: remoteMetadata.size)
                ...
                if locationUrl.host == nil {
                        ...
                        var components = URLComponents(url: originalUrl, resolvingAgainstBaseURL: true)
                    {
                        ...
                        if let resolvedUrl = components.url {
                            ...
                            newRequest.setValue("", forHTTPHeaderField: "If-None-Match")
                            newRequest.setValue("", forHTTPHeaderField: "If-Modified-Since")
