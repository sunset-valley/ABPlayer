...
EXCLUDED_COMMIT_PATTERNS="^ci\(release_sh\):|^Merge pull request|^Merge branch"
...
if [ -f "$STATE_FILE" ]; then
	...
	if git cat-file -e "$LAST_COMMIT" 2>/dev/null; then
		COMMITS=$(git log --no-merges --pretty=format:"%s" "$LAST_COMMIT..HEAD" | grep -vE "$EXCLUDED_COMMIT_PATTERNS")
	else
		echo -e "${RED}Warning: Last commit $LAST_COMMIT not found. Defaulting to last 10 commits.${NC}"
		COMMITS=$(git log --no-merges -n 10 --pretty=format:"%s" | grep -vE "$EXCLUDED_COMMIT_PATTERNS")
	fi
else
...
while IFS= read -r commit; do
	...
	# Extract prefix (handles scoped commits like feat(ui):)
	full_prefix=$(echo "$commit" | grep -oE "^[a-zA-Z]+(\([^\)]+\))?:" | tr '[:upper:]' '[:lower:]')
	prefix=$(echo "$full_prefix" | sed -E 's/^([a-zA-Z]+).*/\1/')

	# Remove prefix from message for cleaner output
	message=$(echo "$commit" | sed -E 's/^[a-zA-Z]+(\([^\)]+\))?: *//')
